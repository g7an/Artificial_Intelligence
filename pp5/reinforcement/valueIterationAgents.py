...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state):
                    ...
                    cur_value = [self.getQValue(state, a) for a in actions]
                    new_values[state] = max(cur_value)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        qvalue = 0
        for next_state, prob in transition_prob:
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(state):
                actions = self.mdp.getPossibleActions(state)
                cur_value = [self.getQValue(state, action) for action in actions]
                self.values[state] = max(cur_value)
